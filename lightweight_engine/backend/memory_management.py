...
def compute_model_gpu_memory_when_using_cpu_swap(current_free_mem, inference_memory):
    ...
    # Более агрессивная стратегия для больших GPU (80GB VRAM)
    if current_free_mem > 50 * 1024 * 1024 * 1024:  # Если больше 50GB свободно
        suggestion = max(
            maximum_memory_available / 1.15,  # Используем 87% вместо 77%
            maximum_memory_available - 2048 * 1024 * 1024  # Оставляем 2GB буфер вместо 1.25GB
        )
        print(f"[Memory] Большой GPU: используем агрессивную стратегию, ", end="")
    else:
        suggestion = max(
...
def load_models_gpu(models, memory_required=0, hard_memory_preservation=0):
    ...
    for loaded_model in models_to_load:
        ...
        if lowvram_available and (vram_set_state == VRAMState.LOW_VRAM or vram_set_state == VRAMState.NORMAL_VRAM):
            ...
            # Более агрессивная стратегия для больших GPU - переходим в LOW_VRAM только при реальной нехватке памяти
            memory_threshold = -1024 * 1024 * 1024 if current_free_mem > 50 * 1024 * 1024 * 1024 else 0  # -1GB для больших GPU
            
            if estimated_remaining_memory < memory_threshold:
                ...
                if previously_loaded > 0:
                    model_gpu_memory_when_using_cpu_swap = max(previously_loaded, model_gpu_memory_when_using_cpu_swap)
                print("Switching to LOW_VRAM mode, ", end="")
            else:
                print("Keeping in NORMAL_VRAM mode, ", end="")
